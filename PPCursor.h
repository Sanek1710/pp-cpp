...
class MicroParser {
 ...
 public:
  ...
  std::string process() {
    ...
    while (c0) {
      ...
      if (at_line_start && !std::isspace(c0)) at_line_start = false;
      append(c0);
      safe_shift();
    }
 ...
 private:
  ...
  State get_state() {
    return {
        .ahead = ahead, .last_line_offset = line_start_offset, .nline = nline};
  }

  void set_state(State state) {
    ahead = state.ahead;
    line_start_offset = state.last_line_offset;
    nline = state.nline;
    c0 = code[ahead - 1];
    c1 = code[ahead];
  }
  ...
  void skip_multiline_comment(bool fill = true) {
    ...
    if (fill) {
      append(nline - start.line, '\n');
      append(end_offset - std::max(start.offset, line_start_offset), ' ');
    }
    dump(code.substr(start.offset, end_offset - start.offset), start.line,
         start.column);
  }
  ...
  void skip_string_like_literal() {
    ...
    if (was_cat) {
      ...
      append(end_offset - line_start_offset, ' ');
    }
    dump(code.substr(start.offset, end_offset - start.offset), start.line,
         start.column);
  }
