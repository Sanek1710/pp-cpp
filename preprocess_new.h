...
class Preprocessor {
  ...
  void preprocess_tkz_tokens(Tokeniser& tkz) {
    buf.clear();
    out.clear();
    while (tkz.eof()) {
      ...
      switch (token.tag) {
        case tag::identifier: {
          ...
          IndexListTail arg_chunk{arg_rages};
          const size_t nprescanned =
              prescan_tkz_macro(token, tkz, *oMacroStamp, arg_chunk);
          if (nprescanned == 0) break;
          expand_m4acro(token,  *oMacroStamp, input, arg_chunk, buffer, output);
          input.remove_prefix(nprescanned);
          size -= nprescanned;
          continue;
        }
